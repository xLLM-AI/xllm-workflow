...
manifest 字段参考 `../../references/run-manifest-template.md`。
...
```text
Use model-pr-optimization-history to query prior work for <model_name>,
...
算子迁移先用 `../xllm-npu-op-migration/SKILL.md`。只有 profiling 证明需要深入
kernel 级优化时，才使用 `../../kernel-pilot/SKILL.md`。
...
| Code | `xllm-npu-op-migration`, `kernel-pilot`, 目标仓库本地 skill | 一个 patch 或实验 |
...
| Record | `xllm-npu-sota-loop`, `humanize/`, `model-pr-optimization-history` | attempt ledger、optimization ledger、可复用经验 |
