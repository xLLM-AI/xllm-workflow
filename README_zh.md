...
| [`op-migration`](prompts/xllm-npu-op-migration-prompts.md) | 算子迁移、torch_npu/Triton-Ascend/AscendC、xllm_ops runtime 接入 |
...
skills/             → 13 个过程化 agent skill（评测、profiler、benchmark、算子迁移…）
reference/
...
**`skills/`** 包含 13 个过程化 agent skill，每个 SKILL.md 定义了执行流程、证据合约和本地 reference。方式 1 会把它们链接到生成的 `.agents/skills`；方式 2 会把它们链接到所选 agent 的 skills 目录。
